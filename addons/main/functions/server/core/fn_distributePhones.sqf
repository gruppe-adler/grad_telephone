...
private _allPhones = [];
{
    ...
    {
        // only init phone if not added manually
        if (!(_x getVariable ["grad_telephone_isPhone", false])) then {
...

{
    ...
    private _hasPublicPhoneBookEntry = _phone getVariable ["grad_telephone_hasPublicPhoneBookEntry", true];
    private _canOnlyCallNumber = _phone getVariable ["grad_telephone_canOnlyCallNumber", "all"];
