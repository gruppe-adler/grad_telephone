...
[_phone2, _sound] remoteExec ["grad_telephone_fnc_fakeCallPlaySoundLocal", allPlayers select { _x != player}];

diag_log format ["Debug: fakeCallPlaySound - playing fake sound locally"];

[{
    params ["_unit", "_phone1", "_phone2", "_soundID"];
    soundParams _soundID isEqualTo [] || ([_unit, _phone2] call grad_telephone_fnc_conditionEndInteraction) ||
    !([_phone2, "calling"] call grad_telephone_fnc_callGetStatus)
}, {
    params ["_unit", "_phone1", "_phone2", "_soundID"];
